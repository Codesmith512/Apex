...
namespace io
{
  namespace screen
  {
    ...
    void vga_screen::event(keyboard::event_t const& e)
    {
      static int i = 0;
      write("Got event " + std::to_string(i++) + "\n");
    }
     ...
     */
    static std::vector<vga_manager*> managers;
    ...
    vga_manager::~vga_manager()
    {
      ...
      for(std::vector<vga_manager*>::iterator it = managers.begin();
          it != managers.end();
          )
      {
        if(*it == this)
    ...
    /* Delegates the event to all managers */
    void vga_manager::global_event(keyboard::event_t const& e)
    {
      for(vga_manager* m : managers)
        m->event(e);
    }

    /* Manage a delegated event */
    void vga_manager::event(keyboard::event_t const& e)
    {
      if(screens.empty())
        return;

      if(screens.back()->is_active())
        screens.back()->event(e);
    }
  }
}
